...
// Gateway initialized (Stripe is optional in local/dev setups)
  ...
  try {
    if (!stripe) {
      return res.json({
        success: false,
        message:
          "Stripe is not configured on the server. Please set STRIPE_SECRET_KEY in backend/.env and restart the backend.",
      });
    }
